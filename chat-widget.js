...
(function () {
  ...
  function parseWebhookResponse(responseData) {
    ...
    if (responseData && typeof responseData === 'object' && !Array.isArray(responseData)) {
      ...
      // If there is at least one key, return the value of the first key
      if (keys.length > 0) {
        return responseData[keys[0]];
      }
    }
    
    // If it's not an object with one key, or something else, log an error and return a friendly message.
