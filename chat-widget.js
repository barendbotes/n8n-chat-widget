(function () {
  ...
  // These will be overridden by the `window.ChatWidgetConfig` object
  // you inject from your server.
  const defaultConfig = {
    webhook: {
      url: "", // This should be overridden by your config
    },
    branding: {
  ...
  // A simple deep merge function to combine user config with defaults
  function deepMerge(target, source) {
    for (const key in source) {
      if (source[key] instanceof Object && key in target) {
        ...
        // Assign new or primitive values
  ...
  // Use the global config object you are injecting
  ...
  function injectStyles(config) {
    ...
    style.innerHTML = `
      ...
      .chat-widget-button svg { width: 32px; height: 32px; transition: transform 0.3s ease; }
      ...
      .chat-input-area button { background-color: ${primaryColor}; color: white; border: none; border-radius: 8px; padding: 10px 20px; margin-left: 10px; cursor: pointer; font-weight: 600; transition: background-color 0.2s; }
  ...
  function parseMarkdown(text) {
    let html = text;
    // Sanitize first to prevent HTML injection
    ...
    // Wrap lines in <p> tags, handles newlines correctly
  ...
  // Wait for the DOM to be fully loaded before creating the widget
